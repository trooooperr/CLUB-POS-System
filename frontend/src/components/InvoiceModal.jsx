    ...
    <div className="moverlay">
      <div className="mbox invoice-premium-modal">
        ...
        <div className="inv-m-body">
          ...
          {/* ADMIN KOT MANAGEMENT SECTION */}
          {(role === 'admin' || role === 'manager') && o.kotIds && o.kotIds.length > 0 && (
            <div className="share-section-card" style={{ marginTop: '4px' }}>
              <div className="share-header" style={{ color: 'var(--red)', fontSize: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ fontWeight: 'bold' }}>⚙️ ADMIN: KOT MANAGEMENT</span>
                <span style={{ fontSize: '9px', color: 'var(--t2)', background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: '4px' }}>
                  {o.kotIds.length} KOT(s)
                </span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {o.kotIds.map((kot) => (
                  <div key={kot._id} style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: '8px', padding: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', borderBottom: '1px dashed #21262d', paddingBottom: '6px' }}>
                      <span style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--t0)' }}>
                        {kot.kotNo || 'KOT'}
                        <span style={{ fontSize: '9px', color: 'var(--t2)', marginLeft: '6px', fontWeight: 'normal' }}>
                          ({new Date(kot.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })})
                        </span>
                      </span>
                      <button
                        type="button"
                        onClick={async () => {
                          if (window.confirm(`⚠️ WARNING: Are you sure you want to DELETE entire "${kot.kotNo || 'KOT'}"? All items in it will be deleted, and stock will be refunded.`)) {
                            try {
                              await deleteKOT(kot._id, o.tableNo);
                              showToast(`Deleted ${kot.kotNo || 'KOT'} and refunded stock successfully`, 'success');
                            } catch (err) {
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {(kot.items || []).map((item, idx) => (
                        <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '11px' }}>
                          <span style={{ color: 'var(--t1)', textTransform: 'uppercase', fontSize: '10px' }}>{item.name}</span>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <button
                              ...
                              onClick={async () => {
                                if (window.confirm(`Remove 1x "${item.name}" from KOT?`)) {
                                  try {
                                    await removeKOTItem(o._id, item.name, 1);
                            ...
                            <span style={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center', color: 'var(--t0)' }}>{item.quantity}</span>
                            <button
                              type="button"
                              onClick={async () => {
                                if (window.confirm(`Delete all "${item.name}" from this KOT?`)) {
                                  try {
                                    await removeKOTItem(o._id, item.name, item.quantity);
